...
struct mapper {
    ...
    uint8_t
        irq_pending; // set by mapper to request a CPU IRQ; cleared by cpu.irq()
    ...
    uint8_t
        *prg_ram; // pointer into ctx for battery-backed PRG-RAM; NULL if none
    ...
    // PPU sets this to 1 when PPUCTRL bit 5 (sprite_size) is 1 (8×16 mode),
    // and to 0 when sprite_size is 0 (8×8 mode).  Updated on every PPUCTRL
    // write.  MMC5 uses it to decide whether BG fetches should use bank-set B
    // (8×16 mode) or the last-written set (8×8 mode).
    uint8_t ppu_sprite_16;
