...
// Uses loopy v register to determine which tile to fetch from scrolled position
static void render_pixel_level2(uint8_t x, uint8_t y) {
    // Check if background rendering is enabled
    if (!ppu.ppumask.bg_render_enable) {
        // Backdrop color
        if (ppu.frame_buffer) {
    ...
    // Direct tile calculation (no scrolling)
    uint16_t tile_x = x / 8; // 0-31
    uint16_t tile_y = y / 8; // 0-29

    // Nametable address (always $2000 - static view)
    uint16_t nametable_addr = 0x2000 + (tile_y * 32) + tile_x;
    uint8_t tile_id = ppu.nametable[nametable_mirror(nametable_addr)];

    // No fine Y scrolling
    uint8_t adjusted_pixel_y = pixel_y;

    // Fetch pattern data from CHR-ROM
    uint16_t pattern_base = ppu.ppuctrl.bg_pattern_table ? 0x1000 : 0x0000;
    uint16_t pattern_addr = pattern_base + (tile_id * 16) + adjusted_pixel_y;
    ...
    // Fetch attribute byte (same as Level 1 - static nametable $2000)
    uint16_t attr_x = tile_x / 4;
    uint16_t attr_y = tile_y / 4;
    uint16_t attr_addr = 0x23C0 + (attr_y * 8) + attr_x;
    uint8_t attr_byte = ppu.nametable[nametable_mirror(attr_addr)];

    // Palette bit selection
    uint8_t attr_shift = ((tile_y & 0x02) << 1) | (tile_x & 0x02);
    uint8_t palette_index = (attr_byte >> attr_shift) & 0x03;

    // Use correct background palette
    uint8_t palette_addr;
    if (pixel_color == 0) {
        palette_addr = 0; // Backdrop color
    } else {
        palette_addr = (palette_index * 4) + pixel_color;
    }
    uint8_t nes_color = ppu.palette_table[palette_addr];

    // Output to frame buffer
    if (ppu.frame_buffer) {
        ppu.frame_buffer[y * 256 + x] = NES_PALETTE[nes_color & 0x3F];
    }

    // Debug logging for first few pixels
    static int debug_pixel_count = 0;
    if (debug_pixel_count < 8 && y == 0) {
        printf("[LEVEL2-REVERTED] Pixel (%d,%d): tile_x=%d, tile_y=%d, "
               "tile_id=%02X, pixel_color=%d, nes_color=%02X (v was %04X - NOT "
               "USED)\n",
               x, y, tile_x, tile_y, tile_id, pixel_color, nes_color, ppu.v);
        debug_pixel_count++;
    }
}
