 ...
 * test_mapper_011.c
 *
 * Unit tests for the Color Dreams (Mapper 011) implementation.
 *
 * Color Dreams: one switchable 32KB PRG bank (bits 0-1) and one switchable
 * 8KB CHR bank (bits 4-7), both selected by a single write to $8000-$FFFF.
 */
 ...
 * PRG bank i is filled with byte (i + 1); CHR bank i filled with (0x10 + i). */
static struct nes_cartridge *make_cart(uint8_t num_prg, uint8_t num_chr) {
    ...
    hdr->prg_rom_size = num_prg * 2; /* iNES prg size in 16KB units */
    hdr->chr_rom_size = num_chr;     /* iNES chr size in 8KB units */
    cart->hdr = hdr;
    cart->mapper_id = 11;
...
static void test_power_up_state(void) {
    ...
    uint8_t prg = mapper_011_cpu_read(map, 0x8000);
    ASSERT(prg == 1, "power-up: $8000 reads PRG bank 0 (filled with 1)");

    uint8_t chr = mapper_011_ppu_read(map, 0x0000);
    ASSERT(chr == 0x10, "power-up: $0000 reads CHR bank 0 (filled with 0x10)");
}

/* test_prg_bank_switch
 *
 * Bits 0-1 select the 32KB PRG bank. */
static void test_prg_bank_switch(void) {
    printf("\n[test_prg_bank_switch]\n");
    struct mapper *map = make_mapper(4, 4);

    mapper_011_cpu_write(map, 0x8000, 0x02); /* PRG bank 2, CHR bank 0 */
    uint8_t v = mapper_011_cpu_read(map, 0x8000);
    ASSERT(v == 3, "PRG bank 2 selected: $8000 reads 3");

    mapper_011_cpu_write(map, 0x8000, 0x03); /* PRG bank 3 */
    v = mapper_011_cpu_read(map, 0xFFFF);
    ASSERT(v == 4, "PRG bank 3 selected: $FFFF reads 4");
}

/* test_chr_bank_switch
 *
 * Bits 4-7 select the 8KB CHR bank. */
static void test_chr_bank_switch(void) {
    printf("\n[test_chr_bank_switch]\n");
    struct mapper *map = make_mapper(4, 4);

    mapper_011_cpu_write(map, 0x8000, 0x20); /* CHR bank 2 (bits 4-7 = 2), PRG bank 0 */
    uint8_t v = mapper_011_ppu_read(map, 0x0000);
    ASSERT(v == 0x12, "CHR bank 2 selected: $0000 reads 0x12");

    mapper_011_cpu_write(map, 0x8000, 0x30); /* CHR bank 3 */
    v = mapper_011_ppu_read(map, 0x1FFF);
    ASSERT(v == 0x13, "CHR bank 3 selected: $1FFF reads 0x13");
}

/* test_prg_and_chr_independent
 *
 * A single write sets both banks. Confirm PRG bits 0-1 and CHR bits 4-7
 * are independently decoded from the same byte. */
static void test_prg_and_chr_independent(void) {
    printf("\n[test_prg_and_chr_independent]\n");
    struct mapper *map = make_mapper(4, 4);

    /* 0x31: PRG bank = bits 0-1 = 1, CHR bank = bits 4-7 = 3 */
    mapper_011_cpu_write(map, 0x8000, 0x31);

    uint8_t prg = mapper_011_cpu_read(map, 0x8000);
    ASSERT(prg == 2, "0x31 write: PRG bank 1 (filled with 2)");

    uint8_t chr = mapper_011_ppu_read(map, 0x0000);
    ASSERT(chr == 0x13, "0x31 write: CHR bank 3 (filled with 0x13)");
}

/* test_write_anywhere_in_range
 *
 * Writes to any address in $8000-$FFFF update the register. */
static void test_write_anywhere_in_range(void) {
    printf("\n[test_write_anywhere_in_range]\n");
    struct mapper *map = make_mapper(4, 4);

    mapper_011_cpu_write(map, 0xC000, 0x11); /* PRG 1, CHR 1 */
    uint8_t prg = mapper_011_cpu_read(map, 0x8000);
    ASSERT(prg == 2, "write to $C000: PRG bank 1 (filled with 2)");

    uint8_t chr = mapper_011_ppu_read(map, 0x0000);
    ASSERT(chr == 0x11, "write to $C000: CHR bank 1 (filled with 0x11)");
}

/* test_prg_full_32kb_window
 *
 * The 32KB window covers both $8000-$BFFF and $C000-$FFFF from the same bank. */
static void test_prg_full_32kb_window(void) {
    printf("\n[test_prg_full_32kb_window]\n");
    struct mapper *map = make_mapper(4, 4);

    mapper_011_cpu_write(map, 0x8000, 0x02); /* PRG bank 2 */
    uint8_t lo = mapper_011_cpu_read(map, 0x8000);
    uint8_t hi = mapper_011_cpu_read(map, 0xFFFF);
    ASSERT(lo == 3 && hi == 3, "bank 2 spans full $8000-$FFFF window");
}

int main(void) {
    printf("=== Mapper 011 (Color Dreams) unit tests ===\n");
